...
- **Curated-first, full set on demand.** Every Tier-3 glyph editor uses `EmojiSlotPicker` / `EmojiPickerContent` ([EmojiPicker.swift](../Septena/Shell/UI/EmojiPicker.swift)), rendered into the row's leading glyph slot. The default surface is a curated grid (`CuratedEmoji`, seeded from the section starter sets in the plugin files — those stay the source of truth for onboarding suggestions). A free-form field and a "More…" button hand off to the OS's own emoji picker (macOS Character Viewer / iOS emoji keyboard) for the complete, always-current set, so we never bundle a Unicode table. Clearing is always one tap.
...
- **Progress ring** — "how full toward a target / 100%". `ProjectProgressIcon` ([SidebarView.swift](../Septena/Shell/Sidebar/SidebarView.swift)): `progress: Double, tint:, diameter:, lineWidth:, arcTint:`. A faint track at `tint.opacity(0.22)` + a rounded accent arc from 12 o'clock. `arcTint` (optional) colors the arc independently of the track — used to dim a derived, target-less ring to half so it never implies a goal the domain doesn't have. **This is the one canonical ring — every ring in the app uses it.** Projects; the habit/supplement completion-rate ring (`CompletionRateBadge`, smaller/thicker — 14pt / 2.5pt); the Rings homepage cell ([RingsHomepageView.swift](../Septena/Shell/Dashboard/RingsHomepageView.swift), 84pt / 9pt, center label + `arcTint` dimming); and the Sleep score ring ([SleepDestinationView.swift](../Septena/Sections/Sleep/SleepDestinationView.swift), 56pt / 4pt, center value). Do NOT re-roll a `trim(from:)` circle — extend `ProjectProgressIcon` if a new ring needs an axis it doesn't have.
