...
// areas. The spec is explicit: they come "from a curated set, not free-form
// input." Every editor used to hand the user a bare one-character TextField —
// this replaces that with a curated grid that still keeps a free-form fallback
// and a clear option, and renders into the row's leading glyph slot.
//
// Two pieces:
    ...
    HStack(spacing: 10) {
      TextField("Emoji", text: $freeform)
        ...
        .onChange(of: freeform) { _, value in
          ...
          // commits the stable value once. Live-updates the binding so the
          // typed glyph shows immediately; dismissal waits for submit.
