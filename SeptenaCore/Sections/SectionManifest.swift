...
public extension SectionSkill {
  ...
  static let all: [SectionSkill] = [
    .init(
      ...
      tools: [
        .init("tasks_list",          "List by view",
              inputs: "optional: view (today|inbox|upcoming|anytime|someday|completed), limit"),
        .init("tasks_create",        "New task. today:true pins to Today; else inbox unless scheduled/area/project given",
              inputs: "required: title · optional: today, scheduled (YYYY-MM-DD), due (YYYY-MM-DD), area (id), project (id)"),
        .init("tasks_update",        "Patch any subset",
        ...
        .init("tasks_complete",      "Mark done. ERRORS on recurring — those must be done in-app",
              inputs: "required: id"),
      ...
      **"Move my errands to Saturday"**
      ...
      - Don't reference area/project by name; always resolve to id first.
      """
    ),
    .init(
      key: "goals",
